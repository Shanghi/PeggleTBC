-- Most or all TBC private servers won't handle ListChannelByName, so another way to get players in
-- a custom channel was made. These variables/functions would be too much work to use normally in
-- the big mess below, so they're set sometime before needed so they can be used easily later.
local messHelper = {
	invitationChannel=nil,
	inviteList=nil,
	TableInsertOnce=nil,
}

-- table function available in WotLK - another backport may have added it already
...
e.temp4={1,4,5,8}
e.temp5={1,2,3,4}
e.polygonCorners={e.temp1,e.temp2,e.temp1,e.temp3,e.temp1,e.temp1,e.temp1,e.temp4,e.temp4,e.temp1,e.temp1,e.temp5,}e.temp1=nil
e.temp2=nil
e.temp3=nil
e.temp4=nil
e.temp5=nil
e.catcherPolygon={49,-7,51,-9,73,8,51,32,-51,32,-74,8,-51,-9,-49,-7,-49,31,49,31}
e.ranks={5,4,6,5,8,6,9,7,10,8,16,9,20,10,}e.polyTable={}
e.dropInfo={}
e.curvedBrick={
[0]=12,
[10]=1,
...
PeggleData.settings={mouseOnTrans=1,mouseOffTrans=1,showMinimapIcon=true,openFlightStart=true,openDeath=false,openLogIn=true,openDuel=true,closeFlightEnd=false,closeReadyCheck=true,closeCombat=true,closeDuelComplete=false,closePeggleLoot=false,inviteChat=true,inviteRaid=false,inviteMinimap=true,inviteDecline=false,hideOutdated=false,
soundVolume=0,minimapAngle=270,defaultPublish="GUILD",battleGameLimit=5,maxChatLine=3}
...
local cal = date("*t")
local t,l,c,r = cal.wday+1, cal.month, cal.day, cal.year
...
messHelper.TableInsertOnce = o.TableInsertOnce
...
local function Oe(e)e.animated=nil
if(e.isPeg) then
s(d.pegQueue,e)
else
s(d.brickQueue,e)
...
local o,l,a=_e()
local o=h(l+((oe*20+10+a)*100)+1,2)o=o..h(w,4)..h(e.stats[4],4)..h(e.stats[5],4)..h(e.stats[6],4)
...
n.player1.value6=a
n.player1.value=w
SendAddonMessage(t.network.prefix,e.commands[6].."+"..o,"WHISPER",n.name2:GetText())
n:UpdateWinners()
...
SendAddonMessage(t.network.prefix,e.commands[19].."+"..n.id.."+"..a,"WHISPER",n.serverName)
end
else
DEFAULT_CHAT_FRAME:AddMessage(e.locale["_PEGGLE_ISSUE1"])
end
end
end
...
d:SpawnParticleGen(n,o+4,.2,.3,.005,ue-1,ue+1,10,13,0,"spark",1,1,.8)for t=1,10 do
r.trail[t]:Hide()
end
...
local function Ae(o,h,p,T)Qe=o
e.extraInfo=h
de=false
e.speedy=nil
r.fineX=nil
...
wipe(t.ballTracker.actionQueue)
t.ballTracker.fastAdd=true
local o=4+6
if(h) then
o=e.currentView[4]
t.bestScoreCaption:SetText(t.bestScoreCaption.caption1)t.bestScore:Show()
elseif(T) then
o=1
t.bestScoreCaption:SetFormattedText(t.bestScoreCaption.caption3,"30")t.bestScore:Hide()
elseif(p) then
t.bestScoreCaption:SetFormattedText(t.bestScoreCaption.caption2,"4m 59s")t.bestScore:Hide()
else
t.bestScoreCaption:SetText(t.bestScoreCaption.caption1)t.bestScore:Show()
end
for e=1,o do
...
local t=CreateFrame("Frame","PeggleSparks"..t.sparkCount,n,"AnimatedShineTemplate")t:ClearAllPoints()
...
e:SetPoint("Topleft",r+6,-(o-4))e:SetWidth(l)
e:SetHeight(32)
...
t.valueCaption=o:CreateCaption(0,0,"",t,a or 14,HIGHLIGHT_FONT_COLOR.r,HIGHLIGHT_FONT_COLOR.g,HIGHLIGHT_FONT_COLOR.b)t.valueCaption:ClearAllPoints()
...
o.dropdownSkin=t:GetBackdrop()o.dropdownSkinPeggle=t:GetBackdrop()
...
local e
if(type(_G[UIDROPDOWNMENU_OPEN_MENU])=="table") then
e=_G[UIDROPDOWNMENU_OPEN_MENU]
elseif(type(_G[UIDROPDOWNMENU_OPEN_MENU])=="string") then
e=getglobal(_G[UIDROPDOWNMENU_OPEN_MENU])
else
e=nil
end
if(e) then
...
local o
if(l==1) then
local e=_G[UIDROPDOWNMENU_OPEN_MENU]
UIDropDownMenu_SetText(n:GetText(),e)
...
local t=_G[UIDROPDOWNMENU_OPEN_MENU]
-- UIDropDownMenu_SetText(text,t) -- n:GetText()
-- UIDropDownMenu_SetSelectedName(t,text) -- n:GetText()
t.selectedValue=n.value
UIDropDownMenu_SetSelectedValue(t, n.value)
...
local function We(o)
o = _G[UIDROPDOWNMENU_INIT_MENU] or this
...
elseif(o.publish) then
	...
	local function AddDropdownItem(text, value)
		wipe(n)
		n.text = text
		n.value = value
		n.arg1 = 1
		n.func = l
		UIDropDownMenu_AddButton(n)
		if not defaultChannel or PeggleData.settings.defaultPublish == value then
			defaultChannel = value
		end
	end

	if IsInGuild() then
		AddDropdownItem(CHAT_MSG_GUILD, "GUILD")
		AddDropdownItem(CHAT_MSG_OFFICER, "OFFICER")
	end
	if GetNumPartyMembers() > 0 then
		AddDropdownItem(CHAT_MSG_PARTY, "PARTY")
	end
	if GetNumRaidMembers() > 0 then
		AddDropdownItem(CHAT_MSG_RAID, "RAID")
		if IsRaidOfficer() then
			AddDropdownItem(CHAT_MSG_RAID_WARNING, "RAID_WARNING")
		end
	end
	AddDropdownItem(CHAT_MSG_YELL, "YELL")
	AddDropdownItem(CHAT_MSG_SAY, "SAY")
	AddDropdownItem(CHAT_MSG_EMOTE, "EMOTE")
	local inInstance, instanceType = IsInInstance()
	if inInstance and instanceType == "pvp" then
		AddDropdownItem(CHAT_MSG_BATTLEGROUND, "BATTLEGROUND")
	end
	local list = {GetChannelList()}
	for i=1,#list do
		if i % 2 == 0 then
			AddDropdownItem(list[i], list[i])
		end
	end
...
UIDropDownMenu_SetAnchor(0,0,t,"Top",t,"Bottom")
CloseDropDownMenus()
...
if(t.publish) then
	...
	UIDropDownMenu_SetSelectedValue(t, t.selectedValue)
end
t.selectedValue=n
UIDropDownMenu_SetWidth(t.menuWidth,t)
end
...
local function ct(e,t)e.down=true
e.highlight:SetAlpha(.2)
end
local function dt(e,t)e.down=nil
if(e.hover==true) then
...
local function rt(e)e.highlight:SetAlpha(0)
e.hover=nil
end
...
local e=(e or t):CreateTexture(nil,"ARTWORK")e:SetPoint("Topleft",o,-n)
e:SetWidth(a)
e:SetHeight(l)
e:Show()
if(i) then
...
t.catcher.Update=function(e,n)e.elapsed=e.elapsed+n
...
n:SetupObject(t,l,a,o,i,d,c,g,s,S,h,f,p)return t
end
...
n:SetupObject(t,S,s,o,d,i,c,h,f,g,p,T,C)return t
end
...
local o=((1+Ee)*o)/2
n.xVel=n.xVel-(o*2*c)n.yVel=n.yVel-(o*2*r)
...
if(c==true) then
if(re>=2) then
d:SpawnParticle(i+1,l,.2,newAngle+180,10,-1,"spark")d:SpawnParticle(i,l-1,.2,newAngle+181,8,-1,"spark")d:SpawnParticle(i-1,l,.2,newAngle+182,6,-1,"spark")d:SpawnParticle(i,l+1,.2,newAngle+183,9,-1,"spark")d:SpawnParticle(i,l,.2,newAngle+184,7,-1,"spark")
end
end
if not(n.hit) then
if(c==true) then
...
local function Ne(o,n)o.elapsed=o.elapsed+n
...
Pe=a
local o=F(a,e.zoomDistance/2)o=o/e.zoomDistance
r:ClearAllPoints()
r:SetPoint(i..n)
r:SetScale(1.5-(o-.5))
J=.1
...
be=n.xVel*g/a+n.yVel*h/a
xe=0
n.x=n.x-(n.xVel*x)n.y=n.y-(n.yVel*x)g=B-n.x
h=W-n.y
a=z(g^2+h^2)
E=g/a
L=h/a
...
pe=pe+(1900+600)*c[2]ge=ge+(1900+600)*c[2]end
ne=1
end
...
o=25+n.nextValue[X]n.bar[o].elapsed=0
...
t.catcherBack:Hide()
if(d.lastPeg) then
d:SpawnParticleGen(d.lastPeg.x,d.lastPeg.y,.5,.3,.005,0,359,1,3,-.05,"spark")d:SpawnParticleGen(d.lastPeg.x,d.lastPeg.y,.5,.3,.005,0,359,1,3,-.05,"spark")
end
t.roundPegs:SetText("")
t.roundPegScore:SetText("")
t.roundPegs:Show()
...
local n=t.catagoryScreen.frames[2]
local o,a,l=_e()
local o=h(a+((oe*20+10+l)*100)+1,2)o=o..h(w,4)..h(e.stats[4],4)..h(e.stats[5],4)..h(e.stats[6],4)
local o=U(o,p(e.name))
n.player1.value1=e.stats[4]
n.player1.value2=e.stats[6]
n.player1.value3=e.stats[5]
...
i=l[a[1]].."+2+"..n
t.network:Send(e.commands[15],i,"WHISPER",d)o=o+8
end
r=l[a[3]]
c=#r
n=""
...
n[t[6]]=true
n[t[7]]=d
--local e,b,T,P=CalendarGetDate()
local cal = date("*t")
local e,b,T,P = cal.wday+1, cal.month, cal.day, cal.year
...
local a=S(c(e,11,12))
local a=c(e,69)
--local e,e,e,e=CalendarGetDate()
local cal = date("*t")
local e,e,e,e = cal.wday+1, cal.month, cal.day, cal.year
...
local function M(t)--(n,a,t,...)
	...
	if(t) then
		if(e.onlineList[t]) then
			...
			local o,n
			local o=Y -- Y=PeggleProfile.challenges
			for l=1,#o do
				n=o[l]
				if(n.serverName==t) then
					...
					if not n.serverName then
						for n=1,#o do
							...
							if(e.onlineList[t]==2) then
								t=gsub(t,"^%l",string.upper)
								SendAddonMessage(e.addonName,l,"WHISPER",t)
							end
						end
					end
				end
...
l=c[n.challengeIndex]if not l.serverName then
i=l.names
...
l=c[n.challengeIndex]l.serverName=nil
...
n.challengeIndex=n.challengeIndex+1
if(n.challengeIndex<=#c) then
l=c[n.challengeIndex]if not l.ended then
...
local r=o:CreateCaption(0,0,"9",c,25,0,1,0,true)r:ClearAllPoints()
...
r=o:CreateCaption(0,0,e.locale["FREE_BALL2"],c,18,0,1,0,true)r:ClearAllPoints()
r:SetPoint("Center",c,"TopLeft",46,-62)
a.freeDisplay1=r
r:Hide()
r=o:CreateCaption(0,0,"25,000",c,12,0,1,0,true)r:ClearAllPoints()
r:SetPoint("Center",c,"TopLeft",46,-62)
a.freeDisplay2=r
r:Hide()
t.ballTracker=a
...
n:SetTexture(e.artPath.."board1")
n:SetTexCoord(unpack(l))
local o=o:CreateCaption(0,0,"25",d,25,1,.69,0,true)o:ClearAllPoints()
...
n:SetPoint("Center",r,"Center",0,-20)n:SetWidth(380)
n:SetHeight(260)
n:EnableMouse(true)
...
n:Hide()
t.summaryScreen=n
n:SetScript("OnShow",function(n)t.fever1:Hide()
t.fever2:Hide()
t.fever3:Hide()
t.feverPegScore:Hide()
n:SetPoint("Center",r,"Center",0,-20)d.temp1=nil
d.temp2=nil
t.rainbow:Hide()
local o=0
...
local t=i(e.stats[3]/e.stats[7]*100)n.title:SetText(e.locale["_SUMMARY_TITLE"..o])
n.best:SetFormattedText(n.best.caption1,P(Z[ee]))n.current:SetFormattedText(n.current.caption1,P(w))n.stat1:SetText(e.stats[1])
...
local l=o:CreateCaption(0,0,"",n,25,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",0,-36)
n.title=l
local r=20
local a=74
l=o:CreateCaption(r,a+24,e.locale["SUMMARY_SCORE_YOURS"],n,23,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",n,"Top",0,-a+2)
l.caption1=l:GetText()
n.current=l
a=a+32-2
local i=17
l=o:CreateCaption(r,a,e.locale["SUMMARY_STAT1"],n,i,1,.82,0,nil,nil)l=o:CreateCaption(r,a,"",n,i,1,1,1,nil,nil)l:ClearAllPoints()
l:SetPoint("Topright",n,"Topleft",r+144,-a)
n.stat1=l
l=o:CreateCaption(r,a+((i+1)*1),e.locale["SUMMARY_STAT2"],n,i,1,.82,0,nil,nil)l=o:CreateCaption(r,a,"",n,i,1,1,1,nil,nil)l:ClearAllPoints()
l:SetPoint("Topright",n,"Topleft",r+144,-a-((i+1)*1))
n.stat2=l
l=o:CreateCaption(r,a+((i+1)*2),e.locale["SUMMARY_STAT3"],n,i,1,.82,0,nil,nil)l=o:CreateCaption(r,a,"",n,i,1,1,1,nil,nil)l:ClearAllPoints()
l:SetPoint("Topright",n,"Topleft",r+144,-a-((i+1)*2))
n.stat3=l
r=170
l=o:CreateCaption(r,a,e.locale["SUMMARY_STAT5"],n,i,1,.82,0,nil,nil)l=o:CreateCaption(r,a,"",n,i,1,1,1,nil,nil)l:ClearAllPoints()
l:SetPoint("Topright",n,"Topleft",r+190,-a)
n.stat5=l
l=o:CreateCaption(r,a+((i+1)*1),e.locale["SUMMARY_STAT6"],n,i,1,.82,0,nil,nil)l=o:CreateCaption(r,a,"",n,i,1,1,1,nil,nil)l:ClearAllPoints()
l:SetPoint("Topright",n,"Topleft",r+190,-a-((i+1)*1))
n.stat6=l
l=o:CreateCaption(r,a+((i+1)*2),e.locale["SUMMARY_STAT4"],n,i,1,.82,0,nil,nil)l=o:CreateCaption(r,a,"",n,i,1,1,1,nil,nil)l:ClearAllPoints()
l:SetPoint("Topright",n,"Topleft",r+190,-a-((i+1)*2))
n.stat4=l
l=o:CreateCaption(r,a,e.locale["SUMMARY_SCORE_BEST"],n,i,1,.82,0,nil,nil)l:ClearAllPoints()
l:SetPoint("Top",n,"Top",0,-a-66)
l.caption1=l:GetText()
n.best=l
n=m(0,0,40,"buttonPublish",true,"sumPublish",S,function(e)t.summaryScreen.publishDuel=nil
...
end,nil,true)n:ClearAllPoints()
n:SetPoint("Bottom",-1,16+44)
S.publish=n
n=m(0,0,40,"buttonOkay",nil,"sumOkay",S,function(e)e.publishInfo=nil
R(false)
if(PeggleData.settings.closePeggleLoot==true) then
t:Hide()
end
end)n:ClearAllPoints()
n:SetPoint("Bottom",0,16)
n:Show()
n=CreateFrame("Frame","",S)
n:SetPoint("Bottom",0,10)n:SetWidth(340)
n:SetHeight(200)
n:EnableMouse(true)
n:SetFrameLevel(n:GetFrameLevel()+3)
S.bragScreen=n
n:Hide()
n:SetScript("OnHide",function(e)t.summaryScreen.publishDuel=nil
...
n.serverChannels={}
n.channelNames={}
n.refreshChannels=function(t,...)wipe(t.serverChannels)
local e
for e=1,#(...)do
...
l=o:CreateCaption(0,0,e.locale["BRAG"],n,i,1,.82,0,nil,nil)l:ClearAllPoints()
l:SetPoint("Top",0,-36)
l:SetWidth(290)
...
-- n.publishDuel=nil
-- n.publishInfo=nil
end,nil,true)e:ClearAllPoints()
e:SetPoint("Bottomleft",16,16)
e=m(0,0,40,"buttonBack",nil,"bragBack",n,function(e)e:GetParent():Hide()
t.summaryScreen:Show()
end,nil,true)e:ClearAllPoints()
...
local function ce()
local n=CreateFrame("Frame","",t.artBorder)
n:SetPoint("Center",r,"Center",0,-20)n:SetWidth(240)
n:SetHeight(214)
n:EnableMouse(true)
...
n:Hide()
t.gameMenu=n
n:SetScript("OnShow",function(n)d:Hide()
...
n:SetScript("OnHide",function(e)d:Show()
end)
local o=o:CreateCaption(0,0,e.locale["MENU"],n,25,1,.82,0,1,nil)o:ClearAllPoints()
o:SetPoint("Top",0,-20)
n=m(0,0,40,"buttonAbandonGame",true,"menuAbandon",l,function(o)
if(t.duelStatus==3) then
local n=t.catagoryScreen.frames[2]
SendAddonMessage(t.network.prefix,e.commands[6],"WHISPER",n.name2:GetText())
n.player1.value=-2
n:UpdateWinners()
if(PeggleData.settings.closeDuelChallenge==true) then
t.duelStatus=nil
...
n:SetPoint("Top",0,-56)
n=m(0,0,40,"buttonRestartLevel",nil,"menuRestart",l,function(t)Ae(Qe,e.extraInfo)
t:GetParent():Hide()
end)n:ClearAllPoints()
n:SetPoint("Top",0,-106)
l.restart=n
n=m(0,0,40,"buttonReturnToGame",nil,"menuReturn",l,function(e)e:GetParent():Hide()
end)n:ClearAllPoints()
...
local function fe()
local n=CreateFrame("Frame","",t.artBorder)
n:SetPoint("Center",r,"Center",0,-20)n:SetWidth(340)
n:SetHeight(260)
n:EnableMouse(true)
n:SetScript("OnLeave",function(e)e:SetAlpha(.25)
end)
n:SetScript("OnEnter",function(e)e:SetAlpha(1)
end)
n:SetScript("OnShow",function(e)e:SetAlpha(1)
end)
local l=e.GetBackdrop()
l.tileSize=128
l.edgeFile="Interface\\Glues\\Common\\TextPanel-Border"l.edgeSize=32
...
n:Hide()
t.charScreen=n
local o=o:CreateCaption(0,0,e.locale["CHAR_SELECT"],n,25,1,.82,0,1,nil)o:ClearAllPoints()
o:SetPoint("Top",0,-20)
local i=function(e,e)
end
...
local r=function(e)e:SetBackdropBorderColor(1,1,.4)
e.hover=true
e:GetParent():SetAlpha(1)
end
local o=function(e)e:SetBackdropBorderColor(.1,.6,.6)
e.hover=nil
...
n:SetPoint("Center",-80,0)n:SetWidth(140)
n:SetHeight(140)
n:SetBackdrop(l)
...
t:SetVertexColor(1,1,1)t:SetPoint("Center",0,1)
n.tex=t
n:EnableMouse(true)
...
t:SetVertexColor(.5,.5,.5)t:SetPoint("Center",0,1)
n.tex=t
n:EnableMouse(true)
n:SetScript("OnMouseDown",i)
n:SetScript("OnMouseUp",d)
...
t:GetParent():Hide()
end)n.onEnter=n:GetScript("OnEnter")
n.onLeave=n:GetScript("OnLeave")
n:SetScript("OnEnter",function(e)e:GetParent():SetAlpha(1)
n:onEnter()
end)
...
n:SetPoint("TopLeft",5,-9)n:SetPoint("BottomRight",-4,4)
...
t=n.talent2
if(o==3) then
t:SetText(t.caption2)t.tex:SetTexCoord(unpack(t.tex.on))
t:SetVertexColor(0,1,0)
else
t:SetText(t.caption1)t.tex:SetTexCoord(unpack(t.tex.off))
t:SetVertexColor(1,1,0)
end
t=n.talent1
if(o>=2) then
...
local l=o:CreateCaption(0,0,e.locale["QUICK_PLAY"],n,40,.05,.66,1,1,nil)l:ClearAllPoints()
l:SetPoint("Top",0,-22)
...
l=o:CreateCaption(0,0,e.locale["SELECT_LEVEL"],c,26,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",c,"Topleft",170,-60)
local r=CreateFrame("Frame","",c)
r:SetPoint("Topleft",20,-90)r:SetPoint("Bottomleft",20,100)
r:SetWidth(300)
...
local s=function(e)a=e:GetParent():GetParent():GetParent()
...
local u,t,u
for a=1,12 do
l=o:CreateCaption(10,10+(a-1)*20,a,r,19,1,1,1,1,nil)l:SetWidth(32)
l:SetHeight(14)
l:SetJustifyH("LEFT")l=o:CreateCaption(42,10+(a-1)*20,e.locale["_LEVEL_NAME"..a],r,19,1,1,1,1,nil)l:SetWidth(300-42-42)
l:SetHeight(14)
...
n=CreateFrame("Frame","",a)
n:SetPoint("Topright",-96+32-10-6,-64)n:SetWidth(192-20-4)
...
a.levelImage=t
n=CreateFrame("Frame","",a)
n:SetPoint("Topright",-34,-236)n:SetWidth(256)
n:SetHeight(60)
n:SetBackdrop(d)
n:SetBackdropColor(0,0,0,.5)
n:SetBackdropBorderColor(1,1,1)
a.bestFrame=n
l=o:CreateCaption(0,0,e.locale["PERSONAL_BEST"],n,20,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Center",0,12)
l:Hide()
a.best=l
l=o:CreateCaption(0,0,"",n,20,1,1,1,1,nil)l:ClearAllPoints()
l:SetPoint("Center",0,-10)
l:Hide()
a.points=l
l=o:CreateCaption(0,0,e.locale["NO_SCORE"],n,20,0,1,0,1,nil)l:ClearAllPoints()
l:SetPoint("Center")
l:SetWidth(220)
a.beatLevel=l
n=CreateFrame("Frame","",a)
n:SetPoint("Topright",a.bestFrame,"Bottomright",0,-4)n:SetWidth(256)
n:SetHeight(60)
n:SetBackdrop(d)
n:SetBackdropColor(0,0,0,.5)
n:SetBackdropBorderColor(1,1,1)
a.mostRecentFrame=n
l=o:CreateCaption(0,0,e.locale["MOST_RECENT"],n,20,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Center",0,12)
l:Show()
a.mostRecent=l
l=o:CreateCaption(0,0,"",n,20,1,1,1,1,nil)l:ClearAllPoints()
...
l=o:CreateCaption(0,0,e.locale["BEAT_THIS_LEVEL1"],a.bestFrame,14,0,1,0,nil,nil)l:ClearAllPoints()
l:SetPoint("Topleft",t,"Topright",0,-8)
l:SetWidth(220)
l.caption1=e.locale["BEAT_THIS_LEVEL1"]l.caption2=e.locale["BEAT_THIS_LEVEL3"]a.talent1=l
l.tex=t
...
t.on=r
t.off=e.artCut["bannerSmall3"]
l=o:CreateCaption(0,0,e.locale["BEAT_THIS_LEVEL2"],a.bestFrame,14,0,1,0,nil,nil)l:ClearAllPoints()
l:SetPoint("Topleft",t,"Topright",0,-8)
l:SetWidth(220)
l.caption1=e.locale["BEAT_THIS_LEVEL2"]l.caption2=e.locale["BEAT_THIS_LEVEL4"]a.talent2=l
l.tex=t
n=m(0,0,64,"buttonGo",nil,"quickPlayGo",a,function(t)e.extraInfo=nil
...
n.showID=1
n:Hide()
n:SetScript("OnShow",function(e)t.levelList:SetParent(e)
t.levelList:UpdateList()
t.levelList:Hide()
...
n.UpdateDisplay=function(n,o)n.showID=o or n.showID
n.levelImage:SetTexture(e.artPath.."bg"..n.showID.."_thumb")
...
n=CreateFrame("Frame","",a)
n:SetPoint("Topright",-35,-200)n:SetWidth(256)
n:SetHeight(128)
a.duelInfo1=n
local l=o:CreateCaption(0,0,e.locale["DUEL"],n,40,.05,.66,1,1,nil)l:ClearAllPoints()
l:SetPoint("Top",a,"Top",0,-22)
l:SetParent(n)
n=CreateFrame("Frame","",a.duelInfo1)
n:SetPoint("Topright",a,"Topright",-98,-60)n:SetWidth(128)
n:SetHeight(128)
n:SetBackdrop(r)
n:SetBackdropColor(0,0,0,0)
n:SetBackdropBorderColor(1,1,1)
...
a.levelImage=tex
l=o:CreateCaption(0,0,e.locale["OPPONENT"],a.duelInfo1,20,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",n,"Bottom",2,-10)
l.caption1=l:GetText()
l.caption2=e.locale["DUEL_CHALLENGE"]
a.name1=l
l=Je(0,0,256,"opponentName",a.duelInfo1,nil,nil,nil,tooltipText)l:ClearAllPoints()
...
a.nameDrop=i
l=o:CreateCaption(0,0,e.locale["WIN_LOSS_PLAYER"],a.duelInfo1,18,1,.82,0,nil,nil)l:ClearAllPoints()
l:SetPoint("Top",n,"Bottom",0,-62)
l.caption1=l:GetText()
a.winLossPlayer=l
l:Hide()
l=o:CreateCaption(0,0,e.locale["OPPONENT_NOTE"].." "..e.locale["OPTIONAL"],a.duelInfo1,20,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",n,"Bottom",0,-100)
l.caption1=l:GetText()
l.caption2=e.locale["OPPONENT_NOTE"]
l.caption3=e.locale["OPPONENT_NOTE2"]
a.note1=l
l=Je(0,0,256,"opponentNote",a.duelInfo1,nil,nil,nil,tooltipText)l:ClearAllPoints()
l:SetPoint("Top",n,"Bottom",0,-120)
l:SetMaxBytes(48)
a.note2=l
l=o:CreateCaption(0,0,"",a.duelInfo1,12,1,1,1,nil,"")l:ClearAllPoints()
...
l:Hide()
a.note2a=l
l=o:CreateCaption(0,0,e.locale["DUEL_STATUS"],a.duelInfo1,24,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",a,"Topleft",170,-120)
l:SetWidth(256)
a.note3Title=l
l:Hide()
l=o:CreateCaption(0,0,"",a.duelInfo1,18,1,1,1,1,nil)l:ClearAllPoints()
...
a.note3=l
l=o:CreateCaption(0,0,e.locale["WIN_LOSS"],n,20,0,1,0,mil,nil)l:ClearAllPoints()
l:SetPoint("Topleft",a,"Bottomleft",20,90)
l:SetWidth(300)
a.winLoss=l
l=o:CreateCaption(0,0,e.locale["WIN_LOSS_LEVEL"],n,12,0,1,0,nil,nil)l:ClearAllPoints()
l:SetPoint("Top",a.winLoss,"Bottom",0,-4)
l:SetWidth(300)
a.winLossLevel=l
l=o:CreateCaption(0,0,"0 - 0",n,26,0,1,0,nil,nil)l:ClearAllPoints()
l:SetPoint("Top",a.winLossLevel,"Bottom",0,-6)
l:SetWidth(300)
a.winLossCount=l
l=o:CreateCaption(0,0,e.locale["DUEL_TIME"],a,14,1,.82,0,nil,nil)l:ClearAllPoints()
...
n.name2:SetJustifyH("CENTER")n.name2:EnableMouse(false)
n.name2:ClearFocus()
...
t.duelTimer:Show()t.duelStatus=1
...
SendAddonMessage(t.network.prefix,e.commands[1].."+"..n.showID.."+"..o.."+"..a,"WHISPER",d)
end
end)n:ClearAllPoints()
n:SetPoint("Top",a.duelInfo1,"Bottom",0,-28)
a.go=n
n=m(0,0,64,"buttonDecline",true,"duelDecline1",a.duelInfo1,function(n)n:Hide()
t.duelTimer:Hide()
...
n.note3:Hide()
n.note3Title:Hide()
SendAddonMessage(t.network.prefix,e.commands[7].."+","WHISPER",n.name2:GetText())
end,nil,true)n:ClearAllPoints()
n:SetPoint("Bottom",a,"Bottom",0,16)
n:Hide()
a.decline1=n
n=m(0,0,64,"buttonOkay",nil,"duelOkay1",a.duelInfo1,function(e)e:Hide()
...
n:Hide()
a.okay1=n
n=m(0,0,64,"buttonGo",nil,"duelOkay2",a.duelInfo1,function(n)n:Hide()
local n=n:GetParent():GetParent()
n.decline2:Hide()
SendAddonMessage(t.network.prefix,e.commands[4].."+","WHISPER",n.name2:GetText())
...
n:Hide()
a.okay2=n
n=m(0,0,64,"buttonDecline",true,"duelDecline2",a.duelInfo1,function(n)n:Hide()
t.duelTimer:Hide()
local n=n:GetParent():GetParent()
n.okay2:Hide()
t.minimap.notice=nil
...
n.note3Title:Hide()
n.go:Show()
SendAddonMessage(t.network.prefix,e.commands[3].."+","WHISPER",n.name2:GetText())
t.duelStatus=nil
end,nil,true)n:ClearAllPoints()
n:SetPoint("Bottom",a,"Bottom",150,16)
n:Hide()
a.decline2=n
n=CreateFrame("Frame","",a)
n:SetPoint("Topright",-35,-160)n:SetWidth(256)
n:SetHeight(128)
n:Hide()
a.duelInfo2=n
l=o:CreateCaption(0,0,e.locale["DUEL_RESULTS"],a.duelInfo2,40,.05,.66,1,1,nil)l:ClearAllPoints()
l:SetPoint("Top",a,"Top",0,-22)
l:SetParent(n)
n=CreateFrame("Frame","",a.duelInfo2)
n:SetPoint("Topleft",a,"Topleft",98,-60)n:SetWidth(128)
n:SetHeight(128)
n:SetBackdrop(r)
n:SetBackdropColor(0,0,0,0)
n:SetBackdropBorderColor(1,1,1)
...
tex:SetHeight(128-14)
a.levelImage2=tex
l=o:CreateCaption(0,0,e.locale["DUEL_RESULT1"],a.duelInfo2,25,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Topleft",n,"Topright",10,-10)
l:SetPoint("Bottomright",a,"Topright",-10,-60-55-10)
l:SetJustifyV("TOP")l.caption1=e.locale["DUEL_RESULT1"]
l.caption2=e.locale["DUEL_RESULT2"]
l.caption3=e.locale["DUEL_RESULT3"]
l.caption4=e.locale["DUEL_RESULT4"]
a.resultStatus=l
l=o:CreateCaption(0,0,e.locale["DUEL_TOTAL_WL"],a.duelInfo2,16,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",a.resultStatus,"Bottom",0,-10)
l:SetWidth(400)
l.caption1=e.locale["DUEL_TOTAL_WL"]a.winLossDuels=l
l=o:CreateCaption(0,0,e.locale["DUEL_OPP_WL"],a.duelInfo2,16,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",a.winLossDuels,"Bottom",0,-6)
l:SetWidth(400)
l.caption1=e.locale["DUEL_OPP_WL"]a.winLossDuelsVs=l
n=CreateFrame("Frame","",a.duelInfo2)
n:SetPoint("Topleft",a,"Topleft",10,-190)n:SetPoint("Topright",a,"Topright",-10,-190)n:SetHeight(88)
n:SetBackdrop(r)
n:SetBackdropColor(0,0,0,.5)
n:SetBackdropBorderColor(1,1,1)
n.value=-1
...
n.value6=0
a.player1=n
l=o:CreateCaption(0,0,e.locale["DUEL_WAITING"],n,25,1,.82,0,1,nil)l:SetAllPoints(n)
l:SetJustifyV("MIDDLE")a.player1.waiting=l
l=o:CreateCaption(0,0,e.locale["DUEL_FORFEIT1"],n,25,1,.82,0,1,nil)l:SetAllPoints(n)
l:SetJustifyV("MIDDLE")a.player1.forfeit=l
...
tex:SetWidth(24)
tex:SetHeight(24)
l=o:CreateCaption(64+32+2,12+4+32+6,"11",n,12,1,1,1,1,nil)l:SetWidth(24)
n.redCount=l
tex=n:CreateTexture(nil,"Artwork")
...
tex:SetWidth(24)
tex:SetHeight(24)
l=o:CreateCaption(64+32+32+2-8,12+4+32+6,"8",n,12,1,1,1,1,nil)l:SetWidth(24)
n.blueCount=l
l=o:CreateCaption(8,12+4,"",n,25,1,.82,0,1,nil)l:SetWidth(n:GetWidth()-16)
l.caption1=e.locale["DUEL_BREAKDOWN1"]n.score=l
l=o:CreateCaption(64+32+8+64-2-7,12+32+8,"",n,18,.05,.66,1,1,nil)l.caption1=e.locale["DUEL_BREAKDOWN2"]n.talent=l
l=o:CreateCaption(64+32+8+64+150-2,12+32+8,"",n,18,.05,.66,1,1,nil)l.caption1=e.locale["DUEL_BREAKDOWN3"]n.style=l
l=o:CreateCaption(64+32+8+64+300,12+32+8,"",n,18,.05,.66,1,1,nil)l.caption1=e.locale["DUEL_BREAKDOWN4"]n.fever=l
n=CreateFrame("Frame","",a.duelInfo2)
n:SetPoint("Topleft",a,"Topleft",10,-190-86)n:SetPoint("Topright",a,"Topright",-10,-190-86)n:SetHeight(88)
n:SetBackdrop(r)
n:SetBackdropColor(0,0,0,.5)
n:SetBackdropBorderColor(1,1,1)
n.value=-1
...
a.player2=n
l=o:CreateCaption(0,0,e.locale["DUEL_WAITING"],n,25,1,.82,0,1,nil)l:SetAllPoints(n)
l:SetJustifyV("MIDDLE")a.player2.waiting=l
l=o:CreateCaption(0,0,e.locale["DUEL_FORFEIT2"],n,25,1,.82,0,1,nil)l:SetAllPoints(n)
l:SetJustifyV("MIDDLE")a.player2.forfeit=l
n=CreateFrame("Frame","",n)
...
n.blueCount=l
l=o:CreateCaption(8,12+4,"",n,25,1,.82,0,1,nil)l:SetWidth(n:GetWidth()-16)
l.caption1=e.locale["DUEL_BREAKDOWN1a"]n.score=l
l=o:CreateCaption(64+32+8+64-2-7,12+32+8,"",n,18,.05,.66,1,1,nil)l.caption1=e.locale["DUEL_BREAKDOWN2"]n.talent=l
l=o:CreateCaption(64+32+8+64+150-2,12+32+8,"",n,18,.05,.66,1,1,nil)l.caption1=e.locale["DUEL_BREAKDOWN3"]n.style=l
l=o:CreateCaption(64+32+8+64+300,12+32+8,"",n,18,.05,.66,1,1,nil)l.caption1=e.locale["DUEL_BREAKDOWN4"]n.fever=l
...
n:SetPoint("Bottom",a,"Bottom",0,16)
a.okay3=n
n=m(0,0,64,"buttonPublish",true,"duelPublish",a.duelInfo2,function(e)t.summaryScreen.publishInfo=e:GetParent():GetParent().publishInfo
...
e:Show()e:SetParent(t)
end)
n:SetScript("OnHide",function(e)t.catagoryScreen.frames[2].timeRemaining:Hide()
end)
n:SetScript("OnUpdate",function(e,n)e.elapsed=e.elapsed+n
if(e.elapsed>=1) then
e.elapsed=0
...
l:SetPoint("TopLeft",5,-9)l:SetPoint("BottomRight",-4,4)
local d=e.GetBackdrop()
d.tileSize=128
d.edgeFile="Interface\\Glues\\Common\\TextPanel-Border"d.edgeSize=32
l:SetBackdrop(d)
...
local p=l
l:Hide()
l:SetScript("OnShow",function(e)e.content1.state=0
e.content2:Hide()
e.content1:Hide()
e.content1:Show()
end)
local a=CreateFrame("Frame","",p)
a:SetPoint("TopLeft")a:SetPoint("BottomRight")
p.content1=a
a.state=0
a:SetScript("OnShow",function(e)t.catagoryScreen.frames[3].content2.active=nil
if(e.state==0) then
local n
for t=1,13 do
e["highlight"..t].tex:SetAlpha(0)
...
if(e.cCount>=PeggleData.settings.battleGameLimit) then
...
n.stageDetails3a:SetText(a[4])n.stageDetails4a:SetText(a[6])
...
local n=o:CreateCaption(0,0,e.locale["CHALLENGE"],a,40,.05,.66,1,1,nil)n:ClearAllPoints()
n:SetPoint("Top",0,-22)
n.caption1=n:GetText()
n.caption2=e.locale["CHALLENGE_DETAILS"]
a.title=n
local i=CreateFrame("Frame","",a)
i:SetPoint("Topleft",10,-58)i:SetPoint("Bottomleft",10,10)
i:SetWidth(200)
d.bgFile=e.artPath.."windowBackground"
d.tileSize=128
i:SetBackdrop(d)
...
i:SetPoint("Topleft",6,-30)i:SetPoint("Bottomright",-6,6+66)
d.bgFile=e.artPath.."windowBackground"
d.tileSize=128
i:SetBackdrop(d)
...
r:SetScript("OnValueChanged",function()
end)
r:SetValue(0)
r:SetScript("OnValueChanged",function(e)t.catagoryScreen.frames[3].content1:UpdateScreen()
end)
r.background=r:CreateTexture(nil,"background")
r.background:SetPoint("TopLeft")r.background:SetPoint("BottomRight",-1,0)r.background:SetTexture(0,0,0,.35)
getglobal(r:GetName().."ScrollUpButton"):SetScript("OnClick",function(e)e:GetParent():SetValue(e:GetParent():GetValue()-1)
end)
getglobal(r:GetName().."ScrollDownButton"):SetScript("OnClick",function(e)e:GetParent():SetValue(e:GetParent():GetValue()+1)
end)
a.listSlider=r
n=o:CreateCaption(20,138,e.locale["CHALLENGE_LIST"],i,16,1,1,0,1,nil)n:ClearAllPoints()
n:SetPoint("Top",i:GetParent(),"Top",0,-10)
n:SetWidth(180)
n.caption1=n:GetText()
n.caption2=e.locale["CHALLENGE_PLAYER"]a.catagory1=n
local C=function(e)
...
local P,h
for t=1,13 do
n=o:CreateCaption(10,10+(t-1)*20,"nameGoesHere",i,11,1,1,1,nil,nil)n:ClearAllPoints()
n:SetPoint("Topleft",10,-10-(t-1)*20)
n:SetWidth(100)
n:SetHeight(14)
n:SetJustifyH("LEFT")
a["info"..t.."1"]=n
n=o:CreateCaption(170,10+(t-1)*20,"48h 44m",i,11,1,1,1,nil,nil)n:ClearAllPoints()
...
t.content2:Show()
end)l:ClearAllPoints()
l:SetPoint("Bottom",0,34)
a.newChallenge=l
-- n=o:CreateCaption(20,138,e.locale["CHALLENGE_LIMIT"],i:GetParent(),10,1,1,1,nil,nil)n:ClearAllPoints()
-- n:SetPoint("Bottom",i:GetParent(),"Bottom",0,10)
-- n:SetWidth(180)
i=CreateFrame("Frame","",a)
i:SetPoint("Topleft",10+200,-58)i:SetPoint("Bottomright",-10,10)
i:SetWidth(200)
d.bgFile=e.artPath.."windowBackground"
d.tileSize=128
i:SetBackdrop(d)
i:SetBackdropColor(.4,.4,.4,1)
i:SetBackdropBorderColor(1,1,1)
n=o:CreateCaption(190,138,e.locale["CHALLENGE_DETAILS"],i,16,1,1,0,1,nil)n:ClearAllPoints()
n:SetPoint("Top",i,"Top",0,-10)
n:SetWidth(180)
a.details=n
n=o:CreateCaption(0,0,e.locale["CHALLENGE_DESC"],i,18,1,.55,0,1,nil)n:ClearAllPoints()
n:SetPoint("Center")
a.noneSelected=n
local r=30
n=o:CreateCaption(12,r,e.locale["CHALLENGE_CAT1"],i,12,1,1,0,nil,nil)a.stageDetails1=n
n=o:CreateCaption(62,r,"nameGoesHere",i,12,1,1,1,nil,nil)n:ClearAllPoints()
n:SetPoint("Topright",i,"Topleft",10+128,-r)
n:SetJustifyH("RIGHT")
a.stageDetails1a=n
r=r+16
n=o:CreateCaption(12,r,e.locale["CHALLENGE_CAT5"],i,12,1,1,0,nil,nil)a.stageDetails2=n
n=o:CreateCaption(82,r,"47h 59m",i,12,1,1,1,nil,nil)n:ClearAllPoints()
n:SetPoint("Topright",i,"Topleft",10+128,-r)
n:SetJustifyH("RIGHT")
a.stageDetails2a=n
r=r+16
n=o:CreateCaption(12,r,e.locale["CHALLENGE_CAT3"],i,12,1,1,0,nil,nil)a.stageDetails3=n
n=o:CreateCaption(82,r,"0",i,12,1,1,1,nil,nil)n:ClearAllPoints()
n:SetPoint("Topright",i,"Topleft",10+128,-r)
n:SetJustifyH("RIGHT")
a.stageDetails3a=n
r=r+16
n=o:CreateCaption(12,r,e.locale["CHALLENGE_CAT2"],i,12,1,1,0,nil,nil)a.stageDetails5=n
n:SetText("")
n=o:CreateCaption(12,r,"",i,12,1,1,1,nil,nil)a.stageDetails5a=n
l=CreateFrame("Frame","",i)
l:SetPoint("Topleft",10,-r)l:SetWidth(128)l:SetHeight(128)l:SetBackdrop(d)
...
h:SetPoint("Center")
h:SetWidth(128-14)h:SetHeight(128-14)l.tex=h
r=r+l:GetHeight()
n=o:CreateCaption(12,r,e.locale["CHALLENGE_CAT6"],i,12,1,1,0,nil,nil)a.stageDetails4=n
r=r+16
n=o:CreateCaption(12,r,"",i,12,1,1,1,nil,nil)n:SetWidth(128)
n:SetHeight(13*4)
n:SetJustifyH("LEFT")
n:SetJustifyV("TOP")
a.stageDetails4a=n
r=r+13*4+4
n=o:CreateCaption(12,r,e.locale["CHALLENGE_CAT4"],i,12,1,1,0,nil,nil)a.stageDetails6=n
r=r+16
n=o:CreateCaption(12,r,e.locale["NOT_PLAYED"],i,12,1,1,1,nil,nil)n.caption1=n:GetText()
...
e.content1:Hide()
e.content1:Show()
end,nil,true)l:ClearAllPoints()
l:SetPoint("Bottom",75,21)
a.backChallenge=l
i=CreateFrame("Frame","",i)
i:SetPoint("Topright",-6,-30)i:SetPoint("Bottomright",-6,6+66)
i:SetWidth(270)
d.bgFile=e.artPath.."windowBackground"
d.tileSize=128
i:SetBackdrop(d)
...
i:SetBackdropBorderColor(1,1,1)
a.nameList=i
i:EnableMouseWheel(true)i:SetScript("OnMouseWheel",function(n,e)a=t.catagoryScreen.frames[3].content1
...
local f,h
for t=1,13 do
n=o:CreateCaption(10,66+(t-1)*20,t,i,11,1,1,1,nil,nil)n:ClearAllPoints()
n:SetPoint("Topleft",12,-10-(t-1)*20)
n:SetWidth(24)
n:SetHeight(14)
n:SetJustifyH("LEFT")
a["infoRank"..t]=n
n=o:CreateCaption(10,66+(t-1)*20,"nameGoesHereSomeMore",i,11,1,1,1,nil,nil)n:ClearAllPoints()
n:SetPoint("Topleft",32,-10-(t-1)*20)
n:SetWidth(100)
n:SetHeight(14)
n:SetJustifyH("LEFT")
a["infoName"..t.."1"]=n
n=o:CreateCaption(170,10+(t-1)*20,"9,999,999",i,11,1,1,1,nil,nil)n:ClearAllPoints()
...
a["infoName"..t.."b"]=h
n=o:CreateCaption(122+30,10+(t-1)*20,"12",i,9,1,1,1,1,nil)n:SetWidth(18)
n:SetHeight(14)
a["infoName"..t.."b1"]=n
h=i:CreateTexture(nil,"Artwork")
h:SetWidth(16)
...
a["infoName"..t.."c"]=h
n=o:CreateCaption(122+46,10+(t-1)*20,"8",i,9,1,1,1,1,nil)n:SetWidth(18)
n:SetHeight(14)
a["infoName"..t.."c1"]=n
end
a:Show()
...
a.setSrc=1
a.setDur=1
a.serverChannels={EnumerateServerChannels()}a.channelNames={}
a.inviteList={}
a.grabNames={}
a.getChannels=function(e)
return ("|cFF4CB2FF"..t.catagoryScreen.frames[3].content2.channelNames[e])
end
n=o:CreateCaption(10,26,e.locale["CHALLENGE_NEW"],a,30,.05,.66,1,1,nil)n.caption1=n:GetText()
n.caption2=e.locale["CHALLENGE"]
n:SetWidth(330)
n=o:CreateCaption(10,60,e.locale["INVITEES"],a,25,1,1,0,1,nil)n:SetWidth(330)
n=o:CreateCaption(10,90,e.locale["CHALLENGE_DESC1"],a,11,1,1,0,1,nil)n:SetWidth(330)
n=o:CreateCaption(10,90,e.locale["CHALLENGE_DESC2"],a,11,1,1,0,nil,nil)n:ClearAllPoints()
n:SetPoint("Bottomleft",10,40)
n:SetWidth(350)
n:Hide()
a.customText=n
n=o:CreateCaption(10,90,e.locale["CHALLENGE_DESC3"],a,10,1,1,1,nil,"")n:ClearAllPoints()
...
end,1,.82,0,tooltipText,true)l:SetHitRectInsets(0,-120,0,0)
l:SetChecked(true)
l:Hide()
a.guildOption1=l
...
local n,n,o,t
e.serverChannels={EnumerateServerChannels()}wipe(e.channelNames)
for n=1,15 do
...
l=x(13,115,e.locale["CHALLENGE_INVITE1"],"inviteSrc1",true,a,f,1,.82,0,tooltipText,true)l:SetHitRectInsets(0,-40,0,0)
l:SetID(1)
l:SetChecked(true)
a.inviteBox=l
l=x(75,115,e.locale["CHALLENGE_INVITE2"],"inviteSrc2",true,a,f,1,.82,0,tooltipText,true)l:SetHitRectInsets(0,-25,0,0)
l:SetID(2)
l=x(123,115,e.locale["CHALLENGE_INVITE3"],"inviteSrc3",true,a,f,1,.82,0,tooltipText,true)l:SetHitRectInsets(0,-50,0,0)
l:SetID(3)
n=o:CreateCaption(320,115,e.locale["INVITED"],a,12,1,1,0,1,nil)n:ClearAllPoints()
n:SetPoint("Bottom",a,"Topleft",280,-130)
n:SetWidth(150)
n.caption=e.locale["INVITED"]
a.invitedCount=n
i=CreateFrame("Frame","",a)
i:SetPoint("Topleft",10,-135)i:SetWidth(160)
i:SetHeight(204)
i:SetBackdrop(d)
...
local f=function(e)e.tex:SetAlpha(0)
end
local b=function(n,e)a=t.catagoryScreen.frames[3].content2
a.nameSlider:SetValue(a.nameSlider:GetValue()-(e*3))
end
local T=function(n,e)a=t.catagoryScreen.frames[3].content2
a.inviteSlider:SetValue(a.inviteSlider:GetValue()-(e*3))
end
local C=function(e)a=t.catagoryScreen.frames[3].content2
...
local P=function(n)a=t.catagoryScreen.frames[3].content2
...
for e=1,15 do
n=o:CreateCaption(14,10+(e-1)*12,"NameGoesHere",i,10,1,1,1,nil,"")n:SetWidth(128)
n:SetHeight(12)
n:SetJustifyH("LEFT")
a["listItemA"..e]=n
l=CreateFrame("Frame","",i)
...
i=CreateFrame("Frame","",a)
i:SetPoint("Topleft",200,-135)i:SetWidth(160)
i:SetHeight(204)
i:SetBackdrop(d)
i:SetBackdropColor(0,0,0,0)
i:SetBackdropBorderColor(1,1,1)
...
l:SetPoint("Topleft",340+16+32-32+16,-46)l:SetWidth(192+32)l:SetHeight(192+32)l:SetBackdrop(d)
l:SetBackdropColor(0,0,0,0)
l:SetBackdropBorderColor(1,1,1)
h=l:CreateTexture(nil,"Background")
h:SetTexture(e.artPath.."bg1_thumb")
h:SetPoint("Center")
h:SetWidth(192+32-14)h:SetHeight(192+32-14)a.levelImage=h
l=We(357,16,256-16,"challengeLevels",nil,nil,a,a.levelImage,nil)getglobal(l:GetName().."Text"):ClearAllPoints()
...
n=o:CreateCaption(383,256+52,e.locale["CHALLENGE_DUR"],a,14,1,1,0,1,nil)n:SetWidth(200)
l=x(383+0,256+68,"12h","duration1",true,a,i,1,1,1,tooltipText)l:SetHitRectInsets(0,-50,0,0)
l:SetID(1)
l:SetChecked(true)
l=x(453+0,256+68,"24h","duration2",true,a,i,1,1,1,tooltipText)l:SetHitRectInsets(0,-50,0,0)
l:SetID(2)
l=x(523+0,256+68,"48h","duration3",true,a,i,1,1,1,tooltipText)l:SetHitRectInsets(0,-50,0,0)
l:SetID(3)
n=o:CreateCaption(373,256+92,e.locale["INVITE_NOTE"].." "..e.locale["OPTIONAL"],a,14,1,.82,0,1,nil)n:SetWidth(220)
n.caption1=n:GetText()
n.caption2=e.locale["INVITE_NOTE"]
a.note1=n
n=Je(383,256+106,194,"inviteNote",a,nil,nil,nil,tooltipText)n:SetMaxLetters(64)
a.note2=n
l=m(0,0,40,"buttonChallenge",nil,"challengeButton",a,function(n)
local n=n:GetParent()
local l=n.inviteList
messHelper.inviteList = l
local a,o
o=1
wipe(n.grabNames)
for e=1,#l do
...
if(e.elapsed==-1) then
messHelper.invitationChannel = u(e.content.grabNames)
for i=1,MAX_CHANNEL_MEMBER_BUTTONS do
local channel,_,_,_,_,_,category = GetChannelDisplayInfo(i)
if channel and channel == messHelper.invitationChannel and category == "CHANNEL_CATEGORY_CUSTOM" then
SetSelectedDisplayChannel(i)
ChannelRoster_Update(i)
break
end
end
...
if(e.elapsed>=2) then
for i=1,MAX_CHANNEL_MEMBER_BUTTONS do
local channel,_,_,_,count,_,category = GetChannelDisplayInfo(i)
if channel and channel == messHelper.invitationChannel and category == "CHANNEL_CATEGORY_CUSTOM" then
for j=1,count do
messHelper.TableInsertOnce(messHelper.inviteList,((GetChannelRosterInfo(i, j))))
end
break
end
end
...
e:Hide()
table.sort(messHelper.inviteList)
...
l:SetPoint("Center")l:SetWidth(560)
l:SetHeight(64)
d.tileSize=128
d.edgeFile="Interface\\Glues\\Common\\TextPanel-Border"d.edgeSize=32
d.bgFile=e.artPath.."windowBackground"
...
n=o:CreateCaption(0,0,e.locale["GENERATING_NAMES"],l,25,1,.82,0,1,nil)n:ClearAllPoints()
n:SetPoint("Center")
...
local function G()
local l=CreateFrame("Frame","",t.catagoryScreen)
l:SetPoint("TopLeft",297,-9)l:SetPoint("BottomRight",-4,4)
local d=e.GetBackdrop()
d.tileSize=128
d.edgeFile="Interface\\Glues\\Common\\TextPanel-Border"d.edgeSize=32
l:SetBackdrop(d)
...
local n=o:CreateCaption(0,0,e.locale["TALENTS"],l,40,.05,.66,1,1,nil)n:ClearAllPoints()
n:SetPoint("Top",0,-20)
n=o:CreateCaption(0,0,e.locale["TALENTS_DESC"],l,18,1,.55,0,nil)n:ClearAllPoints()
n:SetPoint("Top",0,-62)
n:SetWidth(l:GetWidth()-20)
local a=CreateFrame("Frame","",l)
a:SetPoint("Top",0,-114)a:SetWidth(l:GetWidth()-40)
a:SetHeight(230)
a:SetBackdrop(d)
...
l.icon=i
i:Hide()
n=o:CreateCaption(0,0,"",a,22,1,1,1,1)n:ClearAllPoints()
n:SetPoint("Top",22,-10)
n:SetWidth(a:GetWidth()-20)
n:Hide()
l.name=n
n=o:CreateCaption(0,0,"",a,16,1,1,1,nil,true)n:ClearAllPoints()
n:SetPoint("Top",22,-30)
n:SetWidth(a:GetWidth()-20)
n:Hide()
l.rank=n
n=o:CreateCaption(0,0,"",a,14,1,.82,0,nil,true)n:ClearAllPoints()
n:SetPoint("Top",0,-54)
n:SetJustifyH("LEFT")
n:SetWidth(a:GetWidth()-40)
n:Hide()
l.desc=n
n=o:CreateCaption(0,0,TOOLTIP_TALENT_LEARN,a,14,0,1,0,nil,true)n:ClearAllPoints()
n:SetPoint("Bottom",0,16)
n:SetJustifyH("LEFT")
n:SetWidth(a:GetWidth()-40)
n:Hide()
l.learn=n
n=o:CreateCaption(0,0,e.locale["MOUSE_OVER"],a,14,0,1,0,nil,true)n:ClearAllPoints()
n:SetPoint("Center",0,0)
n:SetWidth(a:GetWidth()-120)
l.mouseOver=n
n=o:CreateCaption(0,0,"",l,18,1,.82,0,nil)n:ClearAllPoints()
...
n:SetPoint("Topright",r,"Topleft",8,0)n:SetPoint("Bottomright",r,"Bottomleft",8,0)n:SetWidth(300)
r.tree=n
n:SetBackdrop(d)
...
i=string.format(e.locale["_TALENT"..n.."_DESC"],e.factors[n*2-1]+o*e.factors[n*2])t.desc:SetText(l.."|r"..i.."\n\n"..r)
t.name:Show()
t.rank:Show()
t.desc:Show()
if(s>0)and(a) then
...
n.showID=1
n:Hide()
n:SetScript("OnShow",function(e)e.showID=1
e:UpdateDisplay(e.showID)
end)
...
r:SetPoint("Topleft",10,-10)r:SetPoint("Bottomleft",10,10)
r:SetWidth(180)
l.bgFile=e.artPath.."windowBackground"
l.tileSize=128
r:SetBackdrop(l)
r:SetBackdropColor(0,0,0,.5)
r:SetBackdropBorderColor(1,.8,.45)
local c=function(e)a=e:GetParent():GetParent()
if(a.showID~=e:GetID()) then
e.tex:SetAlpha(.5)
end
end
local d=function(e)a=e:GetParent():GetParent()
if(a.showID~=e:GetID()) then
e.tex:SetAlpha(0)
end
end
...
local t,t,t
for t=1,4 do
text=o:CreateCaption(10,10+(t-1)*20,e.locale["HOW_TO_PLAY"..t],r,19,1,1,1,1,nil)text:SetWidth(180-16)
text:SetHeight(14)
text:SetJustifyH("LEFT")n=CreateFrame("Frame","",r)
n:SetWidth(180-16)
n:SetHeight(20)
...
t:Hide()
a.content1=t
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY1"],t,40,.05,.66,1,1,nil)text:ClearAllPoints()
...
t:Hide()
a.content2=t
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY2"],t,40,.05,.66,1,1,nil)text:ClearAllPoints()
text:SetPoint("Top",t,"Top",0,-22)
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY2a"],t,11,1,1,1,nil,nil)text:SetWidth(t:GetWidth()-20)
text:ClearAllPoints()
...
n:SetTexture(e.artPath.."howtoplay")
n:SetTexCoord(unpack(l))
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY2b"],t,11,1,1,1,nil,nil)text:SetWidth(t:GetWidth()-20)
text:ClearAllPoints()
...
t:Hide()
a.content3=t
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY3"],t,40,.05,.66,1,1,nil)text:ClearAllPoints()
text:SetPoint("Top",t,"Top",0,-22)
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY3a"],t,11,1,1,1,nil,nil)text:SetWidth(t:GetWidth()-20)
text:ClearAllPoints()
text:SetPoint("Top",t,"Top",0,-64)
...
n:SetTexture(e.artPath.."howtoplay")
n:SetTexCoord(unpack(l))
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY3b"],t,11,1,1,1,nil,nil)text:SetWidth(t:GetWidth()-20)
text:ClearAllPoints()
text:SetPoint("Top",n,"Bottom",0,-10)
t=CreateFrame("Frame","",a)
t:SetPoint("Topleft",r,"Topright")
...
t:Hide()
a.content4=t
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY4"],t,40,.05,.66,1,1,nil)text:ClearAllPoints()
text:SetPoint("Top",t,"Top",0,-22)
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY4a"],t,11,1,1,1,nil,nil)text:SetWidth(t:GetWidth()-20)
text:ClearAllPoints()
text:SetPoint("Top",t,"Top",0,-64)
...
n:SetTexture(e.artPath.."howtoplay")
n:SetTexCoord(unpack(l))
text=o:CreateCaption(0,0,e.locale["HOW_TO_PLAY4b"],t,11,1,1,1,nil,nil)text:SetWidth(t:GetWidth()-20)
text:ClearAllPoints()
text:SetPoint("Top",n,"Bottom",0,-10)
return a
end
...
local a=230
local l=100
n=o:CreateSlider(a,l,300,e.locale["OPT_TRANS_DEFAULT"],"mouseOnTrans",d,.1,1,.01,true,function(e)t:SetAlpha(PeggleData.settings.mouseOnTrans)
end)
n:ClearAllPoints()
n:SetPoint("Top",0,-20)
n=o:CreateSlider(a,l,300,e.locale["OPT_TRANS_MOUSE"],"mouseOffTrans",d,0,1,.01,true,nil)n:ClearAllPoints()
n:SetPoint("Top",0,-50)
n=o:CreateSlider(a,l,300,e.locale["OPT_BATTLEGAMELIMIT"],"battleGameLimit",d,5,50,1,false,nil)n:ClearAllPoints()
n:SetPoint("Top",0,-80)
n=o:CreateSlider(a,l,300,e.locale["OPT_MAXCHATLINES"],"maxChatLines",d,3,25,1,false,nil)n:ClearAllPoints()
n:SetPoint("Top",0,-110)

n=x(a,l+28,e.locale["OPT_MINIMAP"],"showMinimapIcon",true,d,function(e)PeggleData.settings[e.key]=(e:GetChecked()==1)
...
local r=function(e)PeggleData.settings[e.key]=(e:GetChecked()==1)
end
n=x(a,l+48,e.locale["OPT_COLORBLIND"],"colorBlindMode",true,d,r,1,.82,0)
--n=x(a,l+48,e.locale["OPT_HIDEOUTDATED"],"hideOutdated",true,d,r,1,.82,0)
n=x(a,l+68,e.locale["OPT_SHOWSPLASH"],"showSplashScreen",true,d,r,1,.82,0)
a=50
l=150+60-8-20 + 15
n=o:CreateCaption(a,l,e.locale["OPT_AUTO_OPEN"],d,14,1,1,0)n=x(a,l+20,e.locale["OPT_AUTO_OPEN1"],"openFlightStart",true,d,r,1,.82,0)
...
a=270
n=o:CreateCaption(a,l,e.locale["OPT_AUTO_CLOSE"],d,14,1,1,0)n=x(a,l+20,e.locale["OPT_AUTO_CLOSE1"],"closeFlightEnd",true,d,r,1,.82,0)
...
a=490
n=o:CreateCaption(a,l,e.locale["OPT_SOUNDS"],d,14,1,1,0)n=x(a,l+20,e.locale["OPT_SOUNDS_NORMAL"],"soundVolume1",true,d,c,1,.82,0)
n:SetID(0)
...
a=50
l=l+120+20 - 8
n=o:CreateCaption(a,l,e.locale["OPT_DUEL_INVITES"],d,14,1,1,0)n=x(a,l+20,e.locale["OPT_DUEL_INVITES1"],"inviteChat",true,d,r,1,.82,0)
...
n=m(360,l+10,44,"buttonAbout",true,"about",d,function(e)t.catagoryScreen.frames[6]:Hide()
t.about:Show()
end,nil,true)
n=m(360,l+60,44,"buttonCredits",true,"credits",d,function(e)t.catagoryScreen.frames[6]:Hide()
t.credits:Show()
end,nil,true)
local l=CreateFrame("Frame","",t.catagoryScreen)
l:SetPoint("TopLeft",5,-9)l:SetPoint("BottomRight",-4,4)
...
l:Hide()
t.about=l
local a=o:CreateCaption(0,0,e.locale["ABOUT"],l,40,.05,.66,1,1,nil)a:ClearAllPoints()
a:SetPoint("Top",0,-20)
...
a=o:CreateCaption(0,0,e.locale["ABOUT_TEXT1"],l,14,1,.85,.1,nil,nil)a:ClearAllPoints()
a:SetPoint("Top",0,-60)
a:SetWidth(l:GetWidth()-60)
a=o:CreateCaption(0,0,e.locale["ABOUT_TEXT4"]..e.versionString,l,14,1,1,1,nil,nil)a:ClearAllPoints()
a:SetPoint("Topright",-20,-20)
a=o:CreateCaption(0,0,e.locale["ABOUT_TEXT3"],l,9,1,1,0,nil,nil)a:ClearAllPoints()
a:SetPoint("Bottom",0,10)
a:SetWidth(l:GetWidth()-60)
a=o:CreateCaption(0,0,e.locale["ABOUT_TEXT2"],l,15,1,.85,.1,nil,nil)a:ClearAllPoints()
a:SetPoint("Bottom",0,80)
a:SetWidth(l:GetWidth()-60)
n=m(0,0,40,"buttonOkay",nil,"aboutOkay",l,function(e)t.catagoryScreen.frames[6]:Show()
e:GetParent():Hide()
end)n:ClearAllPoints()
...
l:Hide()
t.credits=l
a=o:CreateCaption(0,0,e.locale["CREDITS"],l,40,.05,.66,1,1,nil)a:ClearAllPoints()
a:SetPoint("Top",0,-20)
r=e.artCut["peggleBringer"]
c=l:CreateTexture(nil,"Artwork")c:SetPoint("Top",-(l:GetWidth()/4)-20,-10)
...
local i=60
local r=20
a=o:CreateCaption(r,i,e.locale["CREDITS1"],l,16,1,1,0,nil,nil)a=o:CreateCaption(r+20,i+20,e.locale["CREDITS1a"],l,14,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
i=i+25+15*1
a=o:CreateCaption(r,i,e.locale["CREDITS2"],l,16,1,1,0,nil,nil)a=o:CreateCaption(r+20,i+20,e.locale["CREDITS2a"],l,14,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
i=i+25+15*1
a=o:CreateCaption(r,i,e.locale["CREDITS3"],l,16,1,1,0,nil,nil)a=o:CreateCaption(r+20,i+20,e.locale["CREDITS3a"],l,14,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
i=i+25+15*2
a=o:CreateCaption(r,i,e.locale["CREDITS4"],l,16,1,1,0,nil,nil)a=o:CreateCaption(r+20,i+20,e.locale["CREDITS4a"],l,14,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
i=i+25+15*1
a=o:CreateCaption(r,i,e.locale["CREDITS5"],l,16,1,1,0,nil,nil)a=o:CreateCaption(r+20,i+20,e.locale["CREDITS5a"],l,14,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
i=i+25+15*2
a=o:CreateCaption(r,i,e.locale["CREDITS6"],l,16,1,1,0,nil,nil)a=o:CreateCaption(r+20,i+20,e.locale["CREDITS6a"],l,14,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
local r=l:GetWidth()/2
i=60
a=o:CreateCaption(r+10,i,e.locale["CREDITS7"],l,16,1,1,0,nil,nil)a=o:CreateCaption(r+30,i+20,e.locale["CREDITS7a"],l,14,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
i=i+25+15*6+20
a=o:CreateCaption(r+10,i,e.locale["CREDITS8"],l,16,1,1,0,nil,nil)r=l:GetWidth()/3
a=o:CreateCaption(r,i+16,e.locale["CREDITS8a"],l,12,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
a=o:CreateCaption(r+200,i+16,e.locale["CREDITS8b"],l,12,1,.85,.1,nil,nil)a:SetJustifyH("LEFT")
n=m(0,0,40,"buttonOkay",nil,"aboutOkay",l,function(e)t.catagoryScreen.frames[6]:Show()
e:GetParent():Hide()
end)n:ClearAllPoints()
...
n:SetPoint("TopLeft",8,-80)n:SetWidth(t:GetWidth()-16-2)
...
l:SetHeight(20)
l:SetPoint("Bottom")
e.outdatedText=o:CreateCaption(0,0,e.locale["OUT_OF_DATE"],l,11,1,1,1,nil,nil)e.outdatedText:ClearAllPoints()
...
t.splash=n
local t=o:CreateCaption(0,0,e.locale["LEGAL1"],n,12,1,1,1,nil,nil)t:ClearAllPoints()
t:SetPoint("Bottom",n,"Bottom",0,20)
local t=n:CreateTexture(nil,"Artwork")
t:SetWidth(640)t:SetHeight(640)
...
if PeggleData.settings.battleGameLimit == nil then PeggleData.settings.battleGameLimit = 5 end
if PeggleData.settings.maxChatLines == nil then PeggleData.settings.maxChatLines = 3 end
...
hooksecurefunc("TakeTaxiNode",function()t.flying=true
if(PeggleData.settings.openFlightStart==true) then
t:Show()
end
end)
...
l:SetBackdropBorderColor(1,.8,.45)
l:SetBackdropBorderColor(1,1,1)
local n=o:CreateCaption(0,0,"Peggle",l,25,1,.4,0,true,nil,"")n:ClearAllPoints()
...
n:SetPoint("Top",0,-78)
n:SetWidth(160*1.8)
n:SetText(e.locale["LEGAL2"])n:Show()
l.text=n
local e=m(0,0,40,"buttonOkay",nil,"legalOkay",l,function(e)t.legal:Hide()
t.legal.text:SetText("")
t.legal.text=nil
t.legal=nil
t.splash.elapsed=0
PeggleData.legalDisplayed=true
end)e:ClearAllPoints()
...
text:SetWidth(270)
text:SetText(e.locale["OUT_OF_DATE"])text:Show()
local o=m(0,0,40,"buttonOkay",nil,"outOfDateOkay",n,function(t)e.outdatedPop:Hide()
end)o:ClearAllPoints()
...
local a=n
if PeggleData.settings.showSplashScreen then
t.splash:Show()
if not t.legal then
t.splash.elapsed=0
...
-- end
local n=CreateFrame("Button","exhibitA",UIParent)
n:SetScript("OnClick", function() this:Hide() end)
n:Hide()
n:ClearAllPoints()
...
local l=n:CreateTexture(nil, "BACKGROUND")
...
e.artCut=nil
if(a) then
message(e.locale["PEGGLE_ISSUE2"])t:Hide()
t.minimap:Hide()
if(t.legal) then
t.legal:Hide()
end
SlashCmdList["PEGGLE"]=function()
end
SlashCmdList["PEGGLELOOT"]=function()
end
...
t:SetScript("OnDragStart",function(e)e:StartMoving()
end)
t:SetScript("OnDragStop",function(e)e:StopMovingOrSizing()
end)
t:SetScript("OnHide",function(e)
end)
...
t.topCover=n
local r=m(0,0,20,"buttonClose",nil,"closeButton",l,function(e)t:Hide()
end)r:ClearAllPoints()
r:SetPoint("Topright",t,"Topright",2,9)r.highlight:SetWidth(12)
...
end)a:ClearAllPoints()
a:SetPoint("Topright",r,"Topleft",0,0)a.highlight:SetWidth(12)
...
r:SetPoint("Topright",a,"Topleft",0,0)r.highlight:SetWidth(r:GetWidth()-6)
...
t.rightBorder3=n
local r=o:CreateCaption(0,0,"0",a,15,.55,.85,1,true)r:ClearAllPoints()
r:SetPoint("TopRight",a,"TopLeft",234,-47)
Ie=r
r=o:CreateCaption(0,0,e.locale["SCORE"],a,15,.55,.85,1,true)r:ClearAllPoints()
r:SetPoint("TopLeft",a,"TopLeft",112,-47)
r=o:CreateCaption(0,0,"300,000",a,15,.55,.85,1,true)r:ClearAllPoints()
r:SetPoint("TopRight",a,"TopLeft",546,-47)
t.bestScore=r
r=o:CreateCaption(0,0,e.locale["SCORE_BEST"],a,15,.55,.85,1,true)r:ClearAllPoints()
...
t.logoFrame=o
local a=o:CreateTexture(nil,"Artwork")a:SetPoint("Top",-14,32)
...
o:SetScript("OnMouseUp",function(e)t.resizing=nil
t:StopMovingOrSizing()
end)
n=o:CreateTexture(nil,"Artwork")n:SetPoint("Topleft",0,0)
n:SetWidth(32)
n:SetHeight(32)
...
local e=n-((n-o)*(e.elapsed/.5))t:SetAlpha(e)
end
if(e.elapsed>.5) then
e.elapsed=0
if(e.fading) then
...
l:SetScript("OnEnter",function(e)t:SetAlpha(PeggleData.settings.mouseOnTrans)
e.mouseOver=true
e.fading=nil
e.elapsed=0
e:EnableMouse(false)
...
e.particleGenQueue={}
e.particleQueue={}
e.bouncer={0,114,224,330,438,550,1,5,10,5,1}e.Add=pt
e.SpawnBall=Pt
e.SpawnPeg=dt
e.SpawnBrick=ut
e.SpawnGlow=ct
...
local a=o:CreateCaption(0,0,"",n,16,1,1,0,true)a:ClearAllPoints()
a:SetPoint("Center",r,"Center",0,e.boardHeight/4)
t.roundPegs=a
a=o:CreateCaption(0,0,"",n,24,1,1,0,true)a:ClearAllPoints()
a:SetPoint("Center",r,"Center",0,e.boardHeight/4-20)
t.roundPegScore=a
a=o:CreateCaption(0,0,"",n,20,.4,1,1,true)a:ClearAllPoints()
...
n:SetFrameLevel(n:GetFrameLevel()+3)
t.roundBalls=n
n:SetScript("OnShow",function(e)e.elapsed=0
e:SetAlpha(1)
...
n:SetScript("OnUpdate",function(e,t)e.elapsed=e.elapsed+t
...
a=o:CreateCaption(0,0,"",n,25,1,0,0,true)a:ClearAllPoints()
a:SetPoint("Center")
a.caption1=e.locale["BALLS_LEFT1"]
a.caption2=e.locale["BALLS_LEFT2"]
n.text=a
local r=CreateFrame("Frame","",t.artBorder)r:SetPoint("Center")
r:SetWidth(20)
r:SetHeight(20)
...
l:Hide()
t.fever3=l
a=o:CreateCaption(0,0,"",t.roundBonusScore:GetParent(),40,.4,1,1,true)a:ClearAllPoints()
a:SetPoint("Top",l,"Bottom",0,20)
t.feverPegScore=a
end
local function n()physicsWindow=CreateFrame("Frame","PhysicsEditingWindow",t)
...
physicsWindow:Hide()
local e=o:CreateCaption(10,15,"Gravity                       pixels/sec",physicsWindow,12)e:ClearAllPoints()e:SetPoint("Bottomleft",t,"Bottomleft",16,-45-25)e=CreateFrame("EditBox","PeggleEditBox1",physicsWindow,"InputBoxTemplate")
e:SetPoint("Bottomleft",t,"Bottomleft",70,-50-25)e:SetWidth(64)
e:SetHeight(20)
e:Show()
e:IsNumeric(true)
e:SetText(be)
...
e:ClearFocus()
e=o:CreateCaption(200,15,"Max glide hits                        hits",physicsWindow,12)e:ClearAllPoints()e:SetPoint("Bottomleft",t,"Bottomleft",280,-45-25)e=CreateFrame("EditBox","PeggleEditBox2",physicsWindow,"InputBoxTemplate")
e:SetPoint("Bottomleft",t,"Bottomleft",380,-50-25)e:SetWidth(64)
e:SetHeight(20)
e:Show()
e:IsNumeric(true)
e:SetText(Te)
...
e:ClearFocus()
e=o:CreateCaption(10,-15,"Shot Force                          pixels/sec",physicsWindow,12)e:ClearAllPoints()e:SetPoint("Bottomleft",t,"Bottomleft",16,-20-25)e=CreateFrame("EditBox","PeggleEditBox3",physicsWindow,"InputBoxTemplate")
e:SetPoint("Bottomleft",t,"Bottomleft",100,-25-25)e:SetWidth(64)
e:SetHeight(20)
e:Show()
e:IsNumeric(true)
e:SetText(se)
...
e:ClearFocus()
e=o:CreateCaption(200,-15,"Elasticity (0.00 to 1.00 pls)",physicsWindow,12)e:ClearAllPoints()e:SetPoint("Bottomleft",t,"Bottomleft",300,-20-25)e=CreateFrame("EditBox","PeggleEditBox4",physicsWindow,"InputBoxTemplate")
e:SetPoint("Bottomleft",t,"Bottomleft",476,-25-25)e:SetWidth(64)
e:SetHeight(20)
e:Show()
e:IsNumeric(true)
e:SetText(Ee)
...
e:ClearFocus()
x(490,30,"Show block polys","showPolys",true,physicsWindow,function(e)showPoly=e:GetChecked()
end)e=CreateFrame("Button",nil,physicsWindow,"OptionsButtonTemplate")e:SetText("Add Ball")
e:ClearAllPoints()e:SetPoint("BottomRight",physicsWindow,"BottomRight",-10,10)e:SetScript("OnClick",function(e)
if(te<15) then
t.ballTracker:UpdateDisplay(2)
end
end)
...
r:SetScript("OnMouseUp",function()e.speedy=nil
end)
r:SetScript("OnMouseWheel",function(e,t)N=N+t*.1
...
local o=o:CreateCaption(0,0,e.locale["_SPECIAL_NAME1"],l,15,.55,.85,1,true)o:ClearAllPoints()
o:SetPoint("Top",0,-5)
t.powerLabel.text=o
l=CreateFrame("Frame","",t.artBorder)
l:SetWidth(1)
...
l:SetScript("OnUpdate",function(e,n)e.elapsed=e.elapsed+n
if(e.elapsed<.5) then
...
n.highlight:Hide()
n:SetPoint("Center",-(76*k(T(0))),(76*I(T(0))))n:Show()
n.elapsed=0
n:SetScript("OnMouseDown",function(e,t)e.icon:SetPoint("Center",0,0)
if(t=="RightButton") then
e.moving=true
end
end)
n:SetScript("OnMouseUp",function(e,n)e.icon:SetPoint("Center",-1,1)e.moving=nil
...
n:SetScript("OnEnter",function(t)t.highlight:Show()
GameTooltip_SetDefaultAnchor(GameTooltip,UIParent)
GameTooltip:SetText("|cFFFFFFFFPeggle")
GameTooltip:AddLine(e.locale["_TOOLTIP_MINIMAP"])
GameTooltip:Show()
end)
n:SetScript("OnLeave",function(e)e.highlight:Hide()
GameTooltip:Hide()
end)
...
n.serverStage=1
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00"..string.format(e.locale["_PEGGLELOOT_CHAT_REMAINING"],30))
elseif(n.serverStage==1)and(n.serverRemaining<20) then
n.serverStage=2
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00"..string.format(e.locale["_PEGGLELOOT_CHAT_REMAINING"],20))
elseif(n.serverStage==2)and(n.serverRemaining<10) then
n.serverStage=3
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00"..string.format(e.locale["_PEGGLELOOT_CHAT_REMAINING"],10))
end
...
local linesLeft = PeggleData.settings.maxChatLine
if linesLeft > 3 then
...
t.peggleLootTimer=n
n=CreateFrame("Frame","",UIParent)
n:SetPoint("Center")n:SetWidth(310)
n:SetHeight(230)
n:EnableMouse(true)
...
local l=o:CreateCaption(0,0,e.locale["PEGGLELOOT_TITLE"],a,25,1,.82,0,1,nil)l:ClearAllPoints()
l:SetPoint("Top",0,-20)
l=o:CreateCaption(0,0,e.locale["PEGGLELOOT_DESC"],a,17,1,1,1,true,nil)l:ClearAllPoints()
l:SetPoint("Top",0,-50)
l:SetWidth(a:GetWidth()-30)
l:SetJustifyV("TOP")
a.desc=l
l=o:CreateCaption(0,0,e.locale["PEGGLELOOT_DESC"],a,18,1,1,1,nil,nil)l:ClearAllPoints()
l:SetPoint("Top",a.desc,"Bottom",0,-10)
l:SetWidth(a:GetWidth()-30)
l:SetJustifyV("TOP")
a.desc=l
l=o:CreateCaption(0,0,"",a,18,1,1,1,nil,nil)l:ClearAllPoints()
...
i.queue={}
i.Send=function(o,n,e,t,l)s(o.queue,n.."+"..(e or(""))..O(174)..t..O(174)..(l or("")))
end
i.Chat=function(o,t,n,e)s(o.queue,"[Peggle]: "..t..O(174)..n..O(174)..(e or("")))
end
i.throttleCount=0
i.elapsed=0
...
if(c(h,1,1)=="c") then
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00Peggle: "..string.format(e.locale["_OUTDATED"],a,"Battle"))
elseif(c(h,1,1)=="d") then
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00Peggle: "..string.format(e.locale["_OUTDATED"],a,"Duel"))
elseif(c(h,1,2)=="pl") then
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00Peggle: "..string.format(e.locale["_OUTDATED"],a,"Peggle Loot"))
end
end
e.oldUsers[a]=true
return
...
SendAddonMessage(C,r[9],"WHISPER",a)
...
SendAddonMessage(C,r[12].."+"..i.."+"..l.serverName,"WHISPER",a)
break
end
end
end
...
Me(nil,l)
SendAddonMessage(C,r[11].."+"..i,"WHISPER",d)
end
break
end
end
...
t.network:Send(e.commands[14],l[T[1]].."+e+","WHISPER",a)break
end
end
return
end
...
SendAddonMessage(C,r[17].."+"..i,"WHISPER",a)
SendAddonMessage(C,r[10].."+"..i,"WHISPER",a)
...
if not e then
s(u,{["id"]=i,["elapsed"]=60})
SendAddonMessage(C,r[18].."+"..i,"WHISPER",a)
end
return
end
if(h==r[17]) then
...
SendAddonMessage(C,r[3],"WHISPER",a)
else
if(t.duelStatus) then
SendAddonMessage(C,r[5],"WHISPER",a)
else
...
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00Peggle: "..string.format(n.name1.caption2,a))
end
...
SendAddonMessage(C,r[2],"WHISPER",a)
n.name2:SetText(a)
n.name2:DisableDrawLayer("BACKGROUND")
n.name2:SetJustifyH("CENTER")n.name2:EnableMouse(false)
n.name2:ClearFocus()
...
Xe(t.duelTab)t.duelTab.hover=nil
t.levelList:Hide()
end
end
end
...
if(t.lastIndex) then
SendAddonMessage(t.prefix,t.command.."+"..t.data,"WHISPER",t.lastIndex)
t.elapsed=0
end
end
if(n>1) then
...
i.process=""
i.updated=nil
i.list={}i:SetScript("OnUpdate",function(t,n)
...
SendAddonMessage(t.prefix,r[13].."+"..n[o[1]],"WHISPER",a)
else
t.processing=2
t.currentIndex=0
t.elapsed=0
...
a=t.list[t.currentID][3][o]e=e..h(a,2)..h(t.list[t.currentID][1][o],4)
end
end
...
local function T()for t=1,#e.commands do
...
SlashCmdList["PEGGLELOOT"]=function(l)l=strtrim(l)
if not l or(l=="") then
...
if(e[e.newInfo[11]]) then
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00Peggle: "..string.format(e.locale["_PEGGLELOOT_ISACTIVE"],ceil(t.peggleLootTimer.serverRemaining)))
return
end
if(de~=true) then
local e=getglobal("PeggleButton_menuAbandon")
...
SendChatMessage(c,"RAID_WARNING")t.network:Send(e.commands[20],l,"WHISPER",e.name)
n=4
for n=1,n do
...
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00Peggle: "..e.locale["_PEGGLELOOT_NOTMASTERLOOTER"])
end
else
DEFAULT_CHAT_FRAME:AddMessage("|CFFFFDD00Peggle: "..e.locale["_PEGGLELOOT_WRONGMETHOD"])
end
end
local l,l,n
...
SlashCmdList["PEGGLEEDIT"]=function()t:SetAlpha(1)
...
r.emptyFunc=function()
end
r.Disable=function(n,e)n.disabled=e
local n
if(e==true) then
d:Hide()
r.foreground:Hide()
